...
/**
 * Redis-backed recording state repository.
 * Falls back to in-memory if Redis is not available.
 */
export class RecordingStateRepositoryRedis {
  private redisService: RedisService;
  private memoryFallback: Map<string, RecordingState> = new Map();
  private readonly KEY_PREFIX = 'meeting:recording:';
  private readonly TTL_SECONDS = 7 * 24 * 60 * 60; // 7 days (recordings may be accessed later)
  ...
  async set(meetingId: string, state: RecordingState): Promise<void> {
    ...
    if (this.redisService.isAvailable()) {
      ...
      if (success) {
        logger.debug(`Recording state stored in Redis for meeting ${meetingId}`);
        return;
      }
    }
    ...
    logger.debug(`Recording state stored in memory (Redis unavailable) for meeting ${meetingId}`);
  }
  ...
  async get(meetingId: string): Promise<RecordingState | null> {
    ...
    if (this.redisService.isAvailable()) {
      ...
      if (value) {
        try {
          return JSON.parse(value) as RecordingState;
        } catch (error) {
          logger.error('Failed to parse recording state from Redis:', error);
        }
      }
    }
  ...
  async update(meetingId: string, updates: Partial<RecordingState>): Promise<RecordingState | null> {
    ...
    const updated: RecordingState = { ...current, ...updates };
